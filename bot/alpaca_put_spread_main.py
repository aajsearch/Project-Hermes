"""
Phase-1 Alpaca runtime: Bull Put Spread (credit) via a single multi-leg `mleg` limit order.

Kalshi V2 is untouched; this is a separate Python process:
  python3 -m bot.alpaca_put_spread_main
"""
...
def main() -> None:
    ...
    init_alpaca_db()
    cfg = load_alpaca_put_spread_config()

    logger.info(
        "[ALPACA_PUT_SPREAD] starting paper=%s execute=%s underlyings=%s target_credit=%.4f",
        cfg.paper,
        cfg.execute,
